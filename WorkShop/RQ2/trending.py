...
def figure_line_chart(months, total_repos, vulnerable_repos, vulnerable_ratio, unique_token):
    ...
    ax1.plot(months, vulnerable_repos, label="Variable leak", color=colors["vulnerable_repos"], marker='s', linestyle='-.', linewidth=2, markersize=8, alpha=0.9)
...
def figure_line_chart_bar(months, total_repos, vulnerable_repos, vulnerable_ratio, unique_token):
    # 科研风格配色（柔和、专业）
    colors = {
        "total_repos":     (55 / 255, 139 / 255, 255 / 255),
        "vulnerable_repos":     (217 / 255, 73 / 255, 47 / 255),  # 红色（突出风险）
        "unique_token": (255 / 255, 154 / 255, 41 / 255),  # 橙色
        "vulnerable_ratio": "#2ca02c"  # 绿色（表示比例）
    }
    ...
    # 绘制总仓库数量、出现漏洞数量和 Unique token 数量的折线图
    ax1.plot(months, total_repos, label="Total Create Repos", color=colors["total_repos"],
             marker='o', linestyle='--', linewidth=2, markersize=8, alpha=0.9)
    ax1.plot(months, vulnerable_repos, label="Repo with EV", color=colors["vulnerable_repos"],
             marker='s', linestyle='-.', linewidth=2, markersize=8, alpha=0.9)
    ax1.plot(months, unique_token, label="Unique Secret", color=colors["unique_token"],
             marker='^', linestyle=':', linewidth=2, markersize=8, alpha=0.9)
    ...
    # 创建第二个 y 轴，绘制漏洞比例为柱状图
    ax2 = ax1.twinx()
    ax2.bar(months, vulnerable_ratio, color=colors["vulnerable_ratio"], alpha=0.3, label="Leak Ratio")
...
def process_files(json_file, variable_file_path,leak_file):
    # 读取 JSON 文件
    ...
    # 判读该月是不是有variable
    if os.path.exists(variable_file_path):
        print("文件存在")
        with open(variable_file_path, "r", encoding="utf-8") as f:
            variable_list = json.load(f)
        print("总变量数量:", len(variable_list))
    else:
        print("文件不存在")
        return len(repo_list), 0, 0

    # 读取 泄露的token
    if os.path.exists(leak_file):
        repo_list_leakage = pd.read_csv(leak_file)
        secrets = repo_list_leakage['raw'].unique()
        print("出现漏洞数量:", len(secrets))
    else:
        return len(repo_list), len(variable_list), 0
    return len(repo_list),len(variable_list), len(secrets)



# 设定文件路径
folder_path = "../../Data/monthly_spaceId_files/"  # 修改为你的实际路径
...
for file in csv_files:
    ...
    variable_file_path = f"../Data/{time}_space_variables.json"
    leak_file = f"match_results/{time}_matches.csv"

    months.append(time)
    print(time + ":")
    repo_list_num, leak_variable_num, leakage_token_num = process_files(repo_file_path, variable_file_path,leak_file)
    total_repos.append(repo_list_num)
    if leakage_token_num == 0 :
        unique_token.append(1)
    else:
        unique_token.append(leakage_token_num)
    if leak_variable_num == 0:
        vulnerable_repos.append(1)
        vulnerable_ratio.append(0)
    else:
        vulnerable_repos.append(leak_variable_num)
        vulnerable_ratio.append(leakage_token_num/leak_variable_num*100)
    # 在这里添加你的处理逻辑，例如读取 CSV 进行处理


figure_line_chart_bar(months,total_repos,vulnerable_repos,vulnerable_ratio,unique_token)
